...
class ArchivedTransaction(AsyncHashedModel):
    ...
    async def prepare(cls, entries: list[Entry], timestamp: str, auth_scripts: dict = {},
        ...
        for entry in entries:
            ...
            vert(await ArchivedTransaction.query().contains('entry_ids', entry.id).count() == 0,
                 f"entry {entry.id} is already contained within an ArchivedTransaction")
    ...
    async def save(self, tapescript_runtime: dict = {}, reload: bool = False) -> ArchivedTransaction:
